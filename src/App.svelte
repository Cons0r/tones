<script lang=ts>
	...
	import App from "./lib/InternalApp.svelte";
	export let type: "web" | "electron"; // This gets passed down to every compenent via context
	...
	let themestring: "light" | "dark";
	onMount(() => {
		theme.subscribe(t => {
			themestring = t?'light':'dark'
...
<ModalList {modals}>
	<div class="w-screen h-screen hide-bar bg-base-100 flex flex-col max-w-[100vw] max-h-[100vh] overflow-auto">
		<nav class="navbar bg-base-200">
			<button class="btn btn-ghost normal-case text-xl">teno</button>
			<div class="flex-grow"></div>
			<HelpButton />
			<label class="swap swap-rotate btn btn-ghost" title="Theme is set to {themestring}">
